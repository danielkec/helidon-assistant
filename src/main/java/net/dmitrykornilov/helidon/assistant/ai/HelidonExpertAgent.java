...
public interface HelidonExpertAgent {
    ...
    @SystemMessage("""
            ...
            {(summary})
            """)
    JsonObject chat(@V("question") String question, @V("previousSummary") String previousConversationSummary);

    @Output
    static JsonObject createResponse(@V("lastResponse") String lastResponse, @V("nextSummary") String nextSummary) {
        return JSON.createObjectBuilder()
                .add("message", lastResponse)
                .add("summary", nextSummary)
                .build();
    }
}
