...
|`flow-control-timeout` |Duration |`PT15S` |Outbound flow control blocking timeout configured as java.time.Duration
...
See RFC 9113 section 6.5.2 for details.
Default is 8192.
|`max-rapid-resets` |int |`50` |Maximum number of rapid resets(stream RST sent by client before any data have been sent by server).
