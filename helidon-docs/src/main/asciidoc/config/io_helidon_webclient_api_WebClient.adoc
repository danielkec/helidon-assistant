...
|`protocol-preference` |string[&#93; |{nbsp} |List of HTTP protocol IDs by order of preference. If left empty, all discovered providers will be used, ordered by
weight.
...
If TLS is not used, we would attempt an upgrade (or use prior knowledge if configured in protocolConfigs()).
