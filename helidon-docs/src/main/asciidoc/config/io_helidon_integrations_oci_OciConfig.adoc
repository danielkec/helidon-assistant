...
- `config-file` -
    use configuration file of OCI (`home/.oci/config`), uses
...
- `instance-principal` - use identity of the OCI instance the service is running on, uses
    ...
    separate module `helidon-integrations-oci-authentication-resource`
- `oke-workload-identity` - use identity of the OCI Kubernetes workload, uses
    `com.oracle.bmc.auth.okeworkloadidentity.OkeWorkloadIdentityAuthenticationDetailsProvider`, and is available in a
    separate module `helidon-integrations-oci-authentication-oke-workload`
...
|`federation-endpoint` |URI |{nbsp} |Customization of federation endpoint for authentication providers.
...
|`tenant-id` |string |{nbsp} |OCI tenant id for Instance Principal, Resource Principal or OKE Workload.
