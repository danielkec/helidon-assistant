...
|`media-supports` |io.helidon.http.media.MediaSupport[&#93; (service provider interface)

Such as:

 - xref:{rootdir}/config/io_helidon_http_media_jackson_JacksonSupport.adoc[jackson (JacksonSupport)]
 - xref:{rootdir}/config/io_helidon_http_media_jsonb_JsonbSupport.adoc[jsonb (JsonbSupport)]
 - xref:{rootdir}/config/io_helidon_http_media_gson_GsonSupport.adoc[gson (GsonSupport)]

 |{nbsp} |Media supports to use.
