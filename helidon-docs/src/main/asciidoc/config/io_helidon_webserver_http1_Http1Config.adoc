...
Type: link:{javadoc-base-url}/io.helidon.webserver/io/helidon/webserver/http1/Http1Config.html[io.helidon.webserver.http1.Http1Config]
...
|`validate-prologue` |boolean |`true` |If set to false, any query and fragment is accepted (even containing illegal characters).
Validation of path is controlled by validatePath().
...
|[.line-through]#`validate-request-host-header`# |boolean |`true` |*Deprecated* Request host header validation.
When host header is invalid, we return io.helidon.http.Status.BAD_REQUEST_400.

The validation is done according to RFC-3986 (see io.helidon.common.uri.UriValidator). This is a requirement of
